...
class TicketList extends StatelessWidget {
  ...
  final List<CreateOrderResponse> bookingList;
    ...
    return bookingList.isNotEmpty
        ? ListView.builder(
            ...
            itemBuilder: ((BuildContext context, int index) {
              CreateOrderResponse item = bookingList[index];
              return Padding(
                padding: EdgeInsets.only(bottom: spacingUnit(2)),
                child:
                    // wideScreen
                    // ? TicketWideCard(
                    //     from: item.result.flightInfo.first,
                    //     to: item.result.flightInfo.first.arrCityNameEng,
                    //     plane: item.plane,
                    //     price: item.price,
                    //     depart: item.depart,
                    ...
                    TicketCard(
                  from: item.result.flightInfo.first,
                  to: item.result.flightInfo.last,
                  // plane: item.plane,
                  price: item.result.amount,
                  depart: item.result.flightInfo.first.dptDate,
                  arrival: item.result.flightInfo.last.arrDate,
                  createdDate: item.result.createDate,
                  status: item.result.status == "confirm"
                      ? BookStatus.done
                      : BookStatus.canceled,
                  timeLeft: item.result.flightInfo.first.dptDate,
                  bookingCode: item.result.orderNo,
                  showDetail: () {
                    item.result.status != "confirm"
                        ? Get.toNamed(AppLink.ticketDetail, arguments: item)
                        : null;
                  },
                  showBoardingPass: () {
                    // showPassengerList();
  ...
  Widget _emptyList(BuildContext context) {
    ...
    return NoData(
      ...
      title: localization.youDontHaveBooking,
      desc: '',
      primaryAction: () {
        Get.toNamed(AppLink.searchFlight);
      },
      primaryTxtBtn: localization.searchFlight,
    );
  }
}
