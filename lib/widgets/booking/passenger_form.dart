...
class _PassengerFormState extends State<PassengerForm> {
  ...
  void initState() {
    super.initState();
    List<User> initPsg =
        List.generate(widget.totalPassengers - 1, (index) => userInit);
    ...
    return ListView(children: [
        ...
        child: ListView(
            ...
            children: [
              ListTile(
                ...
                trailing: Text(userDummy.username,
      ...
      AppInputBox(
          content: ListView(
              ...
              children: [
            ListTile(
              title: const Text('Same as contact details',
            ...
            ListTile(
              ...
              contentPadding: const EdgeInsets.all(0),
              minTileHeight: 0,
              subtitle: _passenggers[0].id != '0'
                  ? Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                          Text(
                              _passenggers[0]
                                  .username, //'${_passenggers[0].title} ${_passenggers[0].name}'
                              style: ThemeText.headline.copyWith(
                                  color: colorScheme(context).onSurface)),
                          Text('ID Number: ${_passenggers[0].idCard}',
                              style: ThemeText.paragraph),
                        ])
                  : Container(),
              trailing: Icon(
              ...
              itemBuilder: ((BuildContext context, int index) {
                return Padding(
                  ...
                  child: AppInputBox(
                    content: ListTile(
                      minTileHeight: 0,
                      title: Text('Passenger ${index + 2}',
                          style: ThemeText.paragraph.copyWith(
                              color: colorScheme(context).onSurfaceVariant)),
                      contentPadding: const EdgeInsets.all(0),
                      subtitle: _passenggers[index + 1].id != '0'
                          ? Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                  Text(_passenggers[index + 1].username,
                                      style: ThemeText.headline.copyWith(
                                          color:
                                              colorScheme(context).onSurface)),
                                  Text(
                                      'ID Number: ${_passenggers[index + 1].idCard}',
                                      style: ThemeText.paragraph),
                                ])
                          : Container(),
                      trailing: Icon(
                          _passenggers[index + 1].id != '0'
                              ? Icons.edit
                              : CupertinoIcons.add_circled,
                          color: colorScheme(context).primary),
                      onTap: () {
